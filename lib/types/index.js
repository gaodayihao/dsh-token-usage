...
/**
 * DSH 权威推理强度 -> 面板四档。
 * `request/header` 的 header.config.reasoningEffort 是实际下发档位，只在变化时
 * 落盘；返回 undefined 表示档位未知，调用方回退 reasoningTokens 近似。
 */
    ...
    return class TokenUsageService extends _classSuper {
        ...
        foldEvent(sessionId, event) {
            ...
            if (event.type === 'request/header') {
                // 权威推理强度：只在变化时落盘，记住最近一次即为后续 step 的生效值。
                const effort = data?.header?.config?.reasoningEffort;
                if (typeof effort === 'string' && effort)
                    this.currentEffort.set(sessionId, effort);
            }
            else if (event.type === 'assistant/chunk') {
                const chunk = data && data.chunk;
                if (chunk && chunk.type === 'usage' && chunk.usage) {
                    const key = stepKey(sessionId, data.turn, data.step);
                    this.stepUsage.set(key, { usage: chunk.usage, model: null, time: event.time, sessionId, effort: this.currentEffort.get(sessionId) });
                }
            }
                ...
                if (usage) {
                    ...
                    this.stepUsage.set(key, { usage, model, time: event.time, sessionId, effort: this.currentEffort.get(sessionId) });
                }
        ...
        async backfillSession(sp, sessionId) {
            ...
            // Current SessionPersistence exposes open(id, access) -> handle.read();
            // the legacy inspect() that returned { meta, events } is gone.
            const handle = await sp.open(sessionId, 'read');
            try {
                const header = handle.header;
                if (header && header.createdAt) {
                    const existing = this.sessionMeta.get(sessionId);
                    if (existing)
                        existing.createdAt = header.createdAt;
                    else
                        this.sessionMeta.set(sessionId, { createdAt: header.createdAt, messageCount: 0, lastMsgTime: null, segStart: null, maxSeg: 0 });
                }
                ...
                for (const ev of events) {
                    if (ev.type === 'request/header') {
                        const e = ev.data?.header?.config?.reasoningEffort;
