...
function SidebarStatsEntry({ wide }) {
    ...
    return h('div', { className: 'dts-root dts-entry-scope', 'data-theme': resolvedThemeOf() }, h('button', {
        className: 'dts-sidebar-entry' + (wide ? '' : ' rail'),
        type: 'button',
        onClick: () => setOpen(true),
        title: 'Token 统计',
        'aria-label': 'Token 统计',
    }, h(Icon, { name: 'sparkles', size: 16 }), wide ? h('span', { className: 'dts-sidebar-label' }, 'Token 统计') : null));
}
...
function HeaderStatsCapsule() {
    ...
    return h('div', { className: 'dts-root dts-entry-scope', 'data-theme': resolvedThemeOf() }, h('button', {
        ...
        'aria-label': 'Token 统计',
    }, h(Icon, { name: 'sparkles', size: 12 }), h('span', { className: 'dts-header-total' }, data ? fmtTok(data.totalTokens) : '统计中')));
}
...
function TokenStatsPanel() {
    ...
    React.useEffect(() => {
        ...
        const onDocMouseDown = (e) => {
            const t = e.target;
            if (t && t.closest && (t.closest('.dts-avatar-wrap') || t.closest('.dts-user-edit')))
                return;
            setAvatarOpen(false);
    ...
    const commitProfile = () => {
        const next = { avatar: profile.avatar, kind: profile.kind, name: draftName.trim() || 'DSH' };
        setProfile(next);
        saveProfile(next);
        setNameEditing(false);
    };
    const setAvatar = (avatar, kind) => {
        const next = { avatar, kind, name: draftName.trim() || profile.name };
        setProfile(next);
        saveProfile(next);
        setAvatarOpen(false);
    };
    ...
    const tlName = { high: '高', medium: '中', low: '低', off: '关闭' };
    ...
    const insight = h('div', { className: 'dts-col' }, h('div', { className: 'dts-col-title' }, '活动洞察'), insightRows.map(([k, v]) => h('div', { className: 'insight-row', key: k }, h('span', { className: 'k' }, k), h('span', { className: 'v' }, v))));
    const skillList = (s.topSkills || []).map(([n, c]) => [n, c, 'skill']);
    const pluginList = (s.topPlugins || []).map(([n, c]) => [n, c, 'plugin']);
    const merged = [...skillList, ...pluginList].sort((a, b) => b[1] - a[1]).slice(0, 5);
    const plugins = merged.length
        ? merged.map(([name, cnt, kind]) => {
            const short = shortName(name);
            return h('div', { className: 'plugin-row', key: kind + ':' + name }, h('div', { className: 'plugin-icon', style: { background: TOOL_COLORS[hashIdx(name)] } }, kind === 'skill' ? '🧩' : (TOOL_ICONS[name] || TOOL_ICONS[short] || '🔌')), h('span', { className: 'plugin-name', title: name }, `$${short}`), h('span', { className: 'plugin-kind ' + kind }, kind === 'skill' ? 'Skill' : '插件'), h('span', { className: 'plugin-count' }, `${fmtInt(cnt)} 次`));
        })
    ...
    const avatarPicker = avatarOpen ? h('div', { className: 'dts-avatar-picker' }, h('div', { className: 'dts-picker-tabs' }, h('button', {
        ...
        ? h('div', { className: 'dts-avatar-grid' }, AVATAR_CHOICES.map(emoji => h('button', {
            key: emoji,
            type: 'button',
            className: 'dts-avatar-opt' + (profile.kind === 'emoji' && emoji === profile.avatar ? ' active' : ''),
            onClick: () => pickEmoji(emoji),
        }, emoji)))
        : h('div', { className: 'dts-avatar-upload' }, h('button', {
            type: 'button',
            className: 'dts-upload-btn',
            onClick: () => { if (fileInputRef.current)
                fileInputRef.current.click(); },
        }, h(Icon, { name: 'upload', size: 20 }), '选择本地图片'), h('input', {
            ref: fileInputRef,
            type: 'file',
            accept: 'image/*',
            style: { display: 'none' },
            ...
            : null), h('div', { className: 'dts-upload-hint' }, '支持 JPG / PNG / WebP，自动裁剪为圆形头像'))) : null;
    const body = h('div', { className: 'dts-stats-body' }, 
    // 仅当已有数据、且后续刷新失败时提示；首轮加载中不显示任何错误条（由"统计中"徽标表达）。
    (error !== null && data !== null) ? h('div', { className: 'dts-stale-note' }, `刷新失败：${error}，显示上次数据`) : null, h('div', { className: 'dts-top-actions' }, h('button', { className: 'dts-icon-btn', type: 'button', onClick: toggleTheme, title: resolvedTheme === 'dark' ? '切换到浅色' : '切换到深色', 'aria-label': '切换主题' }, h(Icon, { name: resolvedTheme === 'dark' ? 'sun' : 'moon', size: 15 })), h('button', { className: 'dts-icon-btn', type: 'button', onClick: () => setOpen(false), 'aria-label': '关闭' }, h(Icon, { name: 'close', size: 15 }))), h('div', { className: 'dts-user' }, h('div', { className: 'dts-avatar-wrap' }, h('div', { className: 'dts-avatar-ring', onClick: toggleAvatar, title: '点击更换头像' }, h('div', { className: 'dts-avatar' }, avatarDisplay), h('div', { className: 'dts-avatar-hover' }, h(Icon, { name: 'camera', size: 16 }))), avatarPicker), nameEditing
        ? h('div', { className: 'dts-user-edit' }, h('input', {
            ...
            onKeyDown: (e) => { if (e.key === 'Enter')
                commitProfile(); },
        }), h('div', { className: 'dts-edit-actions' }, h('button', { className: 'dts-edit-btn primary', type: 'button', onClick: commitProfile }, '保存'), h('button', { className: 'dts-edit-btn', type: 'button', onClick: () => setNameEditing(false) }, '取消')))
        : h('div', { className: 'dts-user-name', onClick: startNameEdit, title: '点击编辑昵称' }, profile.name, h('span', { className: 'dts-name-edit-icon' }, h(Icon, { name: 'pencil', size: 13 }))), h('div', { className: 'dts-user-sub' }, userSub)), metrics, h(Heatmap, { data, hmMode, setHmMode, tip, setTip, loading }), h('div', { className: 'dts-bottom' }, insight, pluginCol));
    const modal = h('div', {
        className: 'dts-panel',
        onClick: (e) => { if (e.target === e.currentTarget)
            setOpen(false); },
    }, h('div', { className: 'dts-panel-content' }, body));
    return h('div', { className: 'dts-root', 'data-theme': resolvedTheme, style: { pointerEvents: 'auto' } }, modal, tip ? h('div', { className: 'dts-tip', style: { left: tip.x, top: tip.y } }, tip.text) : null);
}
function Heatmap({ data, hmMode, setHmMode, tip: _tip, setTip, loading }) {
            ...
            requestAnimationFrame(() => {
                ...
                if (el) {
                    ...
                    if (x + r.width > window.innerWidth)
                        setTip({ x: e.clientX - r.width - 10, y, text });
                }
            });
        }
    };
    ...
    if (hmMode === 'weekly') {
        ...
        let acc = 0;
        const pts = dayList.map(d => { acc += valOf(d); return [d, acc]; });
        const W = 940;
        const H = 140;
        ...
        const fmtShort = (v) => v >= 100000000 ? `${(v / 100000000).toFixed(1)}亿` : v >= 10000 ? `${(v / 10000).toFixed(0)}万` : String(v);
        const yGrid = yTicks.map((t, i) => h('g', { key: 'y' + i }, h('line', { x1: P.l, y1: t.y, x2: W - P.r, y2: t.y, stroke: 'var(--border)', strokeWidth: 1 }), h('text', { x: P.l - 8, y: t.y + 4, textAnchor: 'end', fontSize: 10, fill: 'var(--text-faint)' }, fmtShort(t.v))));
        const xTicks = [];
        let lastM = -1;
        ...
        body = h('svg', {
            ...
            onMouseMove: onMove,
            onMouseLeave: hideTip,
        }, h('defs', null, h('linearGradient', { id: 'dtsHmArea', x1: 0, y1: 0, x2: 0, y2: 1 }, h('stop', { offset: '0%', stopColor: '#8b7cf6', stopOpacity: 0.32 }), h('stop', { offset: '100%', stopColor: '#8b7cf6', stopOpacity: 0.02 }))), yGrid, h('path', { d: area, fill: 'url(#dtsHmArea)' }), h('path', { d: line, fill: 'none', stroke: '#8b7cf6', strokeWidth: 1.8 }), xLabels);
    }
    else {
        const firstDow = start.getDay();
        const pad = firstDow;
        ...
        body = h('div', null, h('div', { className: 'hm-months' }, monthSpans.map((t, i) => h('span', { key: i }, t))), h('div', { className: 'hm-grid' }, cellArr.map((d, i) => {
            if (!d)
                return h('div', { className: 'hm-cell', key: i });
            const v = valOf(d);
    ...
    const legend = hmMode === 'daily'
        ? h('div', { className: 'hm-legend' }, h('span', null, '少'), [0, 1, 2, 3, 4, 5, 6].map(l => h('div', { className: `hm-cell l${l}`, key: l })), h('span', null, '多'))
        : h('div', { className: 'hm-legend' }, hmMode === 'weekly'
    ...
    return h('div', { className: 'dts-heatmap' }, h('div', { className: 'hm-header' }, h('div', { className: 'hm-title' }, 'Token 活动', loading ? h('span', { className: 'dts-loading-badge', title: '正在统计 Token 用量…' }, h('span', { className: 'dts-loading-dot' }), '统计中') : null), h('div', { className: 'hm-tabs' }, ['daily', 'weekly', 'cumulative'].map(mode => h('button', {
        key: mode,
...
export function apply(ctx) {
    ...
    ctx.effect(() => {
        ...
        const disposeOverlay = ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'token-stats', order: 100, label: 'Token 统计' }, TokenStatsPanel));
        // 入口 1：侧边栏底部（应用级常驻，挨着设置）。
        const disposeSidebar = ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'token-stats-sidebar' }, SidebarStatsEntry));
        // 入口 2：会话头部实时用量胶囊。
        const disposeHeader = ctx.slots.inject('conversation.session.header.actions', () => ctx.slots.register({ name: 'conversation.session.header.actions', id: 'token-stats-header', order: 90 }, HeaderStatsCapsule));
        return () => {
            clearInterval(timer);
            if (typeof disposeOverlay === 'function')
                disposeOverlay();
            if (typeof disposeSidebar === 'function')
                disposeSidebar();
            if (typeof disposeHeader === 'function')
                disposeHeader();
        };
    });
}
