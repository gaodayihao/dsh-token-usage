...
function streakDays(dateStrings) {
	...
	const dayObjs = days.map((d) => /* @__PURE__ */ new Date(`${d}T00:00:00`));
	let longest = 1;
	let run = 1;
	for (let i = 1; i < dayObjs.length; i++) if (Math.round((dayObjs[i].getTime() - dayObjs[i - 1].getTime()) / 864e5) === 1) {
		run += 1;
	...
	const todayGap = Math.round((today.getTime() - latest.getTime()) / 864e5);
	let current = 0;
	if (todayGap <= 1) {
		current = 1;
		for (let i = dayObjs.length - 1; i > 0; i--) if (Math.round((dayObjs[i].getTime() - dayObjs[i - 1].getTime()) / 864e5) === 1) current += 1;
		else break;
	}
...
const CORE_TOOLS = new Set([
	...
	"mcp__reference_memory__search_nodes",
	"mcp__reference_memory__open_nodes",
	"mcp__reference_memory__create_entities",
	"mcp__reference_memory__add_observations",
	"mcp__reference_memory__create_relations",
	"mcp__reference_memory__delete_entities",
	"mcp__reference_memory__delete_observations",
...
*/
let TokenStatsService = (() => {
	let _classSuper = TypertRemoteService;
	let _instanceExtraInitializers = [];
	let _getStats_decorators;
	return class TokenStatsService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _getStats_decorators, {
				...
				private: false,
				access: {
					has: (obj) => "getStats" in obj,
					get: (obj) => obj.getStats
				},
				metadata: _metadata
		...
		stepUsage = (__runInitializers(this, _instanceExtraInitializers), /* @__PURE__ */ new Map());
		/** foldedSeq: sessionId -> 已折叠的最高持久 seq（backfill/live 去重）。 */
		foldedSeq = /* @__PURE__ */ new Map();
		...
		*/
		...
		async [Service.init]() {
			...
			const offLive = this.ctx.on("session/event", (session, event) => {
				...
				if (this.backfillDone || this.foldedSeq.has(sessionId)) this.foldNew(sessionId, event);
				else {
			...
			this.ctx.effect(() => () => {
				offLive();
			}, "dsh-token-stats: live subscription");
			this.runBackfill(sp);
		}
		...
		foldEvent(sessionId, event) {
			...
			} else if (event.type === "user/message") this.bumpMessage(sessionId, event.time);
		...
		bumpMessage(sessionId, time) {
			...
			if (meta.lastMsgTime == null) {
				...
				if ((time - meta.lastMsgTime) / 1e3 > 1800) {
					const segLen = (meta.lastMsgTime - meta.segStart) / 1e3;
		...
		async backfillSession(sp, sessionId) {
			...
			if (meta && meta.createdAt) {
				...
				else this.sessionMeta.set(sessionId, {
		...
		async runBackfill(sp) {
			...
			try {
				...
				for (const header of headers) try {
		...
		longestChatSegment() {
			...
			for (const meta of this.sessionMeta.values()) {
				...
				if (meta.lastMsgTime != null && meta.segStart != null) {
					const tail = (meta.lastMsgTime - meta.segStart) / 1e3;
		...
		snapshot() {
			...
			for (const record of this.stepUsage.values()) {
				...
				const fam = modelFamily(record.model ?? "unknown");
			...
			const sessionDates = [];
			const typeBreakdown = {};
			const sessionIdSet = /* @__PURE__ */ new Set();
			for (const [id, meta] of this.sessionMeta) {
				sessionIdSet.add(id);
				const date = meta.createdAt ? new Date(meta.createdAt) : null;
				if (date && !Number.isNaN(date.getTime())) sessionDates.push(dayKey(date.getTime()));
			...
			const topPlugins = [...this.toolCounts.entries()].filter(([name]) => !CORE_TOOLS.has(name) && !name.startsWith("read") && !name.startsWith("edit") && name !== "skill").sort((a, b) => b[1] - a[1]).slice(0, 5);
...
//#endregion
export { TokenStatsService, TokenStatsService as default };
