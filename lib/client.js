window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		* Token 统计面板 — 浏览器半边（Codex 风格）
		*
		* 以 Codex 个人用量页统计面板为样本，逻辑与视觉均为 DSH 原生实现：
		...
		*/
		...
		function hmLevelsOf(vals) {
			const sorted = vals.filter((v) => v > 0).sort((a, b) => a - b);
			const lv = [];
			if (sorted.length) for (let i = 1; i <= 6; i++) {
...
`;
		...
		*/
		...
		function TokenStatsPanel() {
			...
			const metrics = h("div", { className: "dts-metrics" }, h("div", { className: "dts-metric" }, h("div", { className: "m-num accent" }, fmtTok(s.totalTokens)), h("div", { className: "m-label" }, "累计 Token")), h("div", { className: "dts-metric" }, h("div", { className: "m-num" }, fmtTok(s.peakTokens)), h("div", { className: "m-label" }, "单会话峰值")), h("div", { className: "dts-metric" }, h("div", { className: "m-num" }, s.maxDurationText || "0分"), h("div", { className: "m-label" }, "最长聊天时长")), h("div", { className: "dts-metric" }, h("div", { className: "m-num" }, s.currentStreak ?? 0, h("span", { className: "unit" }, "天")), h("div", { className: "m-label" }, "当前连续")), h("div", { className: "dts-metric" }, h("div", { className: "m-num" }, s.longestStreak ?? 0, h("span", { className: "unit" }, "天")), h("div", { className: "m-label" }, "最长连续")));
			...
			const mbPct = mbTop && mbTotal ? Math.round(mbTop[1] / mbTotal * 100) : 0;
			const insightRows = [
			...
			}, h("div", { className: "dts-panel-content" }, h("div", { className: "dts-stats-body" }, error !== null ? h("div", { className: "dts-stale-note" }, data !== null ? `刷新失败：${error}，显示上次数据` : `加载失败：${error}`) : null, h("div", { className: "dts-top-actions" }, h("button", {
		...
		function Heatmap({ data, hmMode, setHmMode, tip: _tip, setTip, loading }) {
			...
			const showTip = (e, d, kind) => {
				const v = valOf(d);
				positionTip(e, kind === "w" ? `${d.getMonth() + 1}月${d.getDate()}日 当周使用了 ${fmtTok(weekSum(d))} Token` : `${d.getMonth() + 1}月${d.getDate()}日 使用了 ${fmtTok(v)} Token`);
			};
			let body;
			if (hmMode === "weekly") {
				const weekSums = [];
			...
			const legend = hmMode === "daily" ? h("div", { className: "hm-legend" }, h("span", null, "少"), [
				...
				key: l
			})), h("span", null, "多")) : h("div", { className: "hm-legend" }, hmMode === "weekly" ? h("span", null, `${dayList.length} 天 · ${weeks} 周`) : h("span", null, `累计 ${fmtTok(windowTotal)} · ${dayList.length} 天`));
